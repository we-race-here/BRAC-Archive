    ...
    <v-row>
      <v-col cols="12" sm="12">
        <v-card outlined color="rgb( 230, 230, 230 )">
          ...
          <v-card-text>
            <p class="text-h5">Cross Team Cup</p>
            <div class="pl-3 mt-3">
              ...
              <v-row>
                ...
                ></v-col>
                <v-col cols="12" sm="1">
                  <v-btn small color="primary" target="__blank__" :href="`/AcaPointscompetitionresult/?ordering=place&pointscompetitionid=${RoadCupCategorySelector.pointscompetitionid}&racegroupid=${RoadCupCategorySelector.racegroupid}&export=csv&type=roadcupteam`" v-if="RoadCupCategorySelector">Export CSV</v-btn>
                </v-col>
              </v-row>
              <v-row>
                <v-col>
...
export default {
  ...
  methods: {
    getCupDetails() {
      CycAxios.get(`/AcaPointscompetition/?name=BCT`).then((data) => {
        console.log(data);
